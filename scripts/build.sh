...
# Build inputstream.ffmpegdirect for a given platform and Kodi version.
#
# Usage:
...
#
# Prerequisites:
#   All platforms:  cmake, make, autopoint, nasm
...
#
# Examples:
#   ./scripts/build.sh --os linux --arch x86_64 --kodi 22 --kodi-src ~/kodi-piers
#   ./scripts/build.sh --os android --arch armv7 --kodi 22 --kodi-src ~/kodi-piers --ndk ~/android-ndk-r25c
...
# pkg-config path for the deps we'll build
...
# Generate toolchain, set cross-compile env, add platform-specific args
...
set(CMAKE_STRIP /usr/bin/arm-linux-gnueabihf-strip CACHE FILEPATH "")
set(CMAKE_FIND_ROOT_PATH_MODE_PROGRAM NEVER)
set(CMAKE_FIND_ROOT_PATH_MODE_LIBRARY ONLY)
set(CMAKE_FIND_ROOT_PATH_MODE_INCLUDE ONLY)
set(CPU arm CACHE STRING "")
set(AUTOTOOLS_HOST arm-linux-gnueabihf CACHE STRING "")
TCEOF
        CMAKE_ARGS+=(
...
set(CMAKE_FIND_ROOT_PATH_MODE_LIBRARY ONLY)
set(CMAKE_FIND_ROOT_PATH_MODE_INCLUDE ONLY)
set(CPU aarch64 CACHE STRING "")
set(AUTOTOOLS_HOST aarch64-linux-gnu CACHE STRING "")
TCEOF
        CMAKE_ARGS+=(
            -DCMAKE_TOOLCHAIN_FILE="$TOOLCHAIN_FILE"
        )
    ...
    android-armv7)
        ...
        export CC="$LLVM_BIN/armv7a-linux-androideabi21-clang"
        export CXX="$LLVM_BIN/armv7a-linux-androideabi21-clang++"
...
set(CPU armv7a CACHE STRING "")
set(AUTOTOOLS_HOST arm-linux-androideabi CACHE STRING "")
...
TCEOF
        ...
        ;;
    android-aarch64)
        echo "  Toolchain: Android NDK ($NDK_PATH) arm64-v8a"
        LLVM_BIN="$NDK_PATH/toolchains/llvm/prebuilt/linux-x86_64/bin"
...
set(ANDROID_PLATFORM android-21 CACHE STRING "" FORCE)
set(ANDROID_STL c++_shared CACHE STRING "" FORCE)
set(CPU arm64-v8a CACHE STRING "")
set(AUTOTOOLS_HOST aarch64-linux-android CACHE STRING "")
include($NDK_PATH/build/cmake/android.toolchain.cmake)
if(DEFINED ADDON_DEPENDS_PATH)
  list(APPEND CMAKE_FIND_ROOT_PATH \${ADDON_DEPENDS_PATH})
  set(Kodi_DIR \${ADDON_DEPENDS_PATH}/lib/kodi CACHE PATH "" FORCE)
...
# Strip the addon shared library — Release builds still ship debug info by
# default, which adds ~4x to the binary size.
case "${TARGET_OS}-${TARGET_ARCH}" in
    linux-x86_64)   STRIP_BIN=strip ;;
    linux-armv7)    STRIP_BIN=arm-linux-gnueabihf-strip ;;
    linux-aarch64)  STRIP_BIN=aarch64-linux-gnu-strip ;;
    android-*)      STRIP_BIN="$NDK_PATH/toolchains/llvm/prebuilt/linux-x86_64/bin/llvm-strip" ;;
esac
echo ""
echo "=== Stripping ==="
find "$INSTALL_DIR/$ADDON_ID" -name "*.so*" -type f -print -exec "$STRIP_BIN" --strip-unneeded {} \;
